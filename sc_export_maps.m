...
% Function: sc_export_maps
% ------------------------
%  Exports static scalp map images from specified time points within a
%  given Stormcloud dataset. 
%
...
function sc_export_maps(thisData,scParams)
%% Global Variables
    ...
    if any(scParams.peaks > totalImages)
        error('Invalid indices: Outside bounds of dataset time axis');
    end
    
    % Specify maxima and minima for mapping to colorbar
    if ~scParams.customThreshFlag
        thisData(thisData == 0) = NaN;
        scParams.dataMin = min(min(min(thisData))); 
        scParams.dataMax = max(max(max(thisData)));
    end
    dataDiff = scParams.dataMax-scParams.dataMin;       
...
%% Initialise New Figure
    ...
    ph_draw_scalp(xDim,yDim,0.001,scParams.scalpShape); % Draw scalp outline     
...
%% Main Program
    ...
    for k = 1:length(scParams.peaks)
        for i = 1:xDim
            for j = 1:yDim
                % Read the current and future values for this pixel                
                thisValue = (thisData(i,j,scParams.peaks(k))- ...
                    scParams.dataMin)/dataDiff;
