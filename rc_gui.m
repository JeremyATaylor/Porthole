...
% Function: rc_gui
% ----------------
%  Graphical user interface (GUI) for Raincloud three-dimensional 
...
function rc_gui
    ...
    global phParams rcParams thisData clusterSize clusterMaxima clusterPeak

    % Initialise new figure
    f = figure('MenuBar','none','NumberTitle','off',...
        'Name','Raincloud Menu','ToolBar','none',...
    ...
    % Start button for launching 3-D Raincloud diagram
    uicontrol(f,'Style','pushbutton','String','Launch Raincloud',...
    ...
    function start_button(~,~,~)
        ...
        raincloud(thisData,rcParams);   % Launch Raincloud
    end
    ...
    function export_button(~,~,~)   
        ...
        if ~rcParams.annotateFlag 
            ...
            ph_export_maps(thisData,rcParams);  % Export scalp maps
        end
    end

    % Anonymous function for extracting all parameters from GUI
    function get_params(~,~,~)
        rcParams = struct;          % Initialise new data structure
        
        % Annotation Flag
        if get(peakCheck,'Value') == get(peakCheck,'Max')
            rcParams.annotateFlag = 1;
        else
            rcParams.annotateFlag = 0;
        end
        
        % Cluster Peaks
        rcParams.peaks = clusterPeak(get(peakList,'Value'));  
        
        % Viewing Angle
        rcParams.viewIndex = get(viewPopup,'Value');
        
        % Scalp Shape 
        shapeString = get(shapePopup,'String');
        rcParams.scalpShape = lower(shapeString{get(shapePopup,'Value')});
        
        % Axis Side
        axisString = get(axisPopup,'String');
        rcParams.axisSide = lower(axisString{get(axisPopup,'Value')});
        
        % Custom Threshold
        if get(customCheck,'Value') == get(customCheck,'Max')
            rcParams.customThreshFlag = 1;
        else
            rcParams.customThreshFlag = 0;
        end

        % Data minimum and maximum
        rcParams.dataMax = str2double(get(maxEdit,'String'));
        rcParams.dataMin = str2double(get(minEdit,'String'));  
    end
